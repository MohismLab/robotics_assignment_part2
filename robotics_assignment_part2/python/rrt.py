...
def adjust_pose(node, final_position, occupancy_grid):
    ...
    def validate_arc(node_a, node_b, center, radius):
        '''
        This function validates whether the arc between node_a.pose and node_b.pose,
         with the circle: center, radius
         was not blocked by obstacles or unknown region
        '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--map', action='store',
                        default='map', help='Which map to use.')
    args, unknown = parser.parse_known_args()

    # Load map.
    with open(args.map + '.yaml') as fp:
        print('Loading map from "{}.yaml":\n{}'.format(args.map, fp))
    ...
    plt.scatter(START_POSE[0], START_POSE[1], s=10,
                marker='o', color='green', zorder=1000)
    plt.scatter(GOAL_POSITION[0], GOAL_POSITION[1],
                s=10, marker='o', color='red', zorder=1000)
